    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <br />
                <label>Kategoriler Arasında Gösterilsin mi ?</label>
                <form id="onay">
                <label style="color:#c2c2c2;"><input type="radio" value="Evet" id="k_evet" name="txtConfirm"/><span style="margin-bottom:-3px; " ></span> Evet</label>
                <label name="göster"style="visibility: hidden;">ccccc</label>
                <label style="color:#c2c2c2;"><input type="radio" value="Hayır" id="k_hayir" name="txtConfirm"/><span style="margin-bottom:-3px; " ></span> Hayır</label>
                </form>
                <br />
                <label>Popüler Kategorilerde Gösterilsin mi ?</label>
                <form id="i_onay">
                <label style="color:#c2c2c2;"><input type="radio" value="Evet" id="p_evet" name="i_txtConfirm"/><span style="margin-bottom:-3px; " ></span> Evet</label>
                <label name="göster"style="visibility: hidden;">ccccc</label>
                <label style="color:#c2c2c2;"><input type="radio" value="Hayır" id="p_hayir" name="i_txtConfirm"/><span style="margin-bottom:-3px; " ></span> Hayır</label>
                </form>
                <br />
             </div>
...
<script>
    $(document).ready(function () {
        ...
        function fetchUser() // This function will fetch data from table and display under <div id="result">
        {
            var action = "Load";
            $.ajax({
                url: "kategori_action.php", //Request send to "kategori_action.php page"
        ...
        $('#modal_button').click(function () {
            ...
            $('#txtName').val(''); 
            $('input:radio[name=i_txtConfirm]:nth(0)').attr('checked',false);
            $('input:radio[name=i_txtConfirm]:nth(1)').attr('checked',false);
            $('input:radio[name=txtConfirm]:nth(0)').attr('checked',false);
            $('input:radio[name=txtConfirm]:nth(1)').attr('checked',false);
        ...
        $('#action').click(function () {
            ...
            var txtConfirm = $('[name=txtConfirm]:checked', '#onay').val(); 
            var i_txtConfirm = $('[name=i_txtConfirm]:checked', '#i_onay').val();//Get the value of last name textbox
            var id = $('#customer_id').val();  //Get the value of hidden field customer id
            var action = $('#action').val();  //Get the value of Modal Action button and stored into action variable
            if (txtName != null && txtConfirm != null && i_txtConfirm != null) //This condition will check both variable has some value
            {
                
                $.ajax({
                    url: "kategori_action.php",    //Request send to "kategori_action.php page"
                    method: "POST",     //Using of Post method for send data
                    data: { txtName: txtName, txtConfirm: txtConfirm ,i_txtConfirm: i_txtConfirm, id: id, action: action }, //Send data to server
                    success: function (data) {
                ...
                alert("Her üç alanda zorunludur.."); //If both or any one of the variable has no value them it will display this message
            }
            $('input:radio[name=i_txtConfirm]:nth(0)').attr('checked',false);
            $('input:radio[name=i_txtConfirm]:nth(1)').attr('checked',false);
            $('input:radio[name=txtConfirm]:nth(0)').attr('checked',false);
            ...
            $.ajax({
                ...
                success: function (data) {
                  ...
                  //It will display modal on webpage
                    ...
                    if( data.confirm == "Evet"){
                        $('input:radio[name=txtConfirm]:nth(0)').attr('checked',true);
                
                    }
                    if( data.confirm == "Hayır"){
                        $('input:radio[name=txtConfirm]:nth(1)').attr('checked',true);
                    
                    }
                    if( data.i_confirm == "Evet"){
                        $('input:radio[name=i_txtConfirm]:nth(0)').attr('checked',true);
                    
                    }
                    if( data.i_confirm == "Hayır"){
                        $('input:radio[name=i_txtConfirm]:nth(1)').attr('checked',true);
